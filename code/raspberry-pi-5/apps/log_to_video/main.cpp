...
#include <filesystem>
#include <iostream>
#include <opencv2/imgproc.hpp>  // <-- ADD THIS
#include <opencv2/videoio.hpp>  // <-- ADD THIS
#include <vector>
...
int main(int argc, char **argv) {
    ...
    // --- ADD THIS BLOCK: Start VideoWriter setup ---

    // Calculate FPS from log timestamps
    double videoFps = 30.0;  // Default FPS
    if (loopTimestamps.size() > 1) {
        auto totalDuration = std::chrono::duration_cast<std::chrono::duration<double>>(loopTimestamps.back() - loopTimestamps.front());
        videoFps = (loopTimestamps.size() - 1) / totalDuration.count();
        if (videoFps < 1.0) videoFps = 1.0;    // Ensure at least 1 FPS
        if (videoFps > 60.0) videoFps = 60.0;  // Cap at 60 FPS
    }
    std::cout << "Calculated video FPS: " << videoFps << std::endl;

    int fourcc = cv::VideoWriter::fourcc('H', '2', '6', '4');  // .mp4 format

    // --- Initialize Lidar VideoWriter ---
    cv::Size lidarFrameSize(800, 800);  // Based on your lidarMat size
    std::string lidarOutputFilename = "lidar_replay.mp4";

    cv::VideoWriter lidarVideoWriter;
    lidarVideoWriter.open(lidarOutputFilename, fourcc, videoFps, lidarFrameSize, true);

    if (!lidarVideoWriter.isOpened()) {
        std::cerr << "Error: Could not open Lidar video: " << lidarOutputFilename << std::endl;
        return 1;
    }
    std::cout << "Recording Lidar to " << lidarOutputFilename << std::endl;

    // --- Initialize Camera VideoWriter (if camera exists) ---
    cv::VideoWriter cameraVideoWriter;
    if (hasCamera) {
        // Use original camera dimensions from your global constants
        cv::Size cameraFrameSize(camWidth, camHeight);
        std::string cameraOutputFilename = "camera_replay.mp4";

        cameraVideoWriter.open(cameraOutputFilename, fourcc, videoFps, cameraFrameSize, true);

        if (!cameraVideoWriter.isOpened()) {
            std::cerr << "Error: Could not open Camera video: " << cameraOutputFilename << std::endl;
            return 1;
        }
        std::cout << "Recording Camera to " << cameraOutputFilename << std::endl;
    }
    ...
    while (true) {
        if (currentTimeIdx < loopTimestamps.size() - 1)
            currentTimeIdx++;
        else
            break;
        ...
        lidarVideoWriter.write(lidarMat);  // <-- ADD THIS
        ...
        if (hasCamera && !timedFrame.frame.empty()) {
            ...
            cameraVideoWriter.write(cameraMat);  // <-- ADD THIS
        }
    }

    lidarVideoWriter.release();   // <-- ADD THIS
    cameraVideoWriter.release();  // <-- ADD THIS (Safe even if not opened)
